...
opts = [
    cfg.StrOpt('fpa_func_test_workdir',
               help='Path where virtualized node disks will be stored.',
               default="/tmp/fpa-func-test/"),
]
CONF = cfg.CONF
...
def _build_disk_path(node_name, disk_name):
    workdir = CONF.fpa_func_test_workdir
...
def create_empty_disk(node_name, disk_name, size):
    ...
    cmd = "/usr/bin/qemu-img " + "create -f raw {path} {size}".format(
        path=path, size=size)
    LOG.info(commands.getoutput(cmd))
    return path
