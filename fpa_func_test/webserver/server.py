...
from fpa_func_test.base import ABS_PATH
    ...
    def do_POST(self):
        ...
        if callback_check is not None:
            callback_file_path = os.path.join(
                CONF.fpa_func_test_workdir, callback_check.group(1),
                'callback')
            open(callback_file_path, 'a').close()
            LOG.info("Got callback request: {0} ".format(self.path))
...
def signal_term_handler(s, f):
    LOG.info("FPA-func-test stub web server terminating ...")
    try:
        httpd.server_close()
    except:
        LOG.error("Cannot close server!")
        sys.exit(1)
    LOG.info("FPA-func-test stub web server has terminated.")
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    LOG.info("="*80)
    LOG.info("FPA-func-test stub webserver started at {0}:{1} "
