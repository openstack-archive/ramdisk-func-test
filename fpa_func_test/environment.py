...
from fpa_func_test.base import TemplateEngine, ABS_PATH
from fpa_func_test.network import Network
from fpa_func_test.node import Node


opts = [
    cfg.StrOpt('image_build_dir',
               default="/tmp/fpa_image_build",
    ...
    cfg.StrOpt('tenant_images_dir',
               default="/tmp/fpa_golden_images",
               help='A path where mock web-server will take tenant images '
                    '(in case of special keyword in URL). '
                    'Swift deploy-driver use is for getting of tenant-image'),
    cfg.StrOpt('kernel',
               default='vmlinuz',
...
class Environment(object):
    ...
    def _setup_webserver(self, port=HTTP_PORT):
        ...
        LOG.info("Starting stub webserver (at IP {0} port {1}, path to tenant "
                 "images folder is '{2}')".format(
            self.network.address, port, self.tenant_images_dir))

        # TODO make webserver singletone (for parallelism's purpose)
    ...
    def _get_rsync_tenant_image_url(self, image_name):
        ...
        if os.path.exists(image_path):
            ...
            raise Exception(
                    "There is no such file '{0}' in '{1}'".format(
    ...
    def _delete_workdir(self):
        LOG.info("Deleting workdir {0}".format(CONF.fpa_func_test_workdir))
        shutil.rmtree(CONF.fpa_func_test_workdir)
