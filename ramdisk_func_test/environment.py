...
class Environment(object):
    ...
    def add_pxe_config_for_current_node(self, public_key):
        ...
        pxe_config = template.render(
            ...
            api_url="http://{0}:{1}".format(self.network.address,
                                            self.HTTP_PORT),
            key_path="{0}:{1}".format(self.network.address, public_key)
        )
    ...
    def _setup_webserver(self, port=HTTP_PORT):
        LOG.info(
            "Starting stub webserver (at IP {0} port {1}, path to tenant "
            "images folder is '{2}')".format(
                self.network.address, port, CONF.tenant_images_dir))
